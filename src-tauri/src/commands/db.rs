...
#[tauri::command]
pub fn db_execute_query(path: String, query: String, state: State<AppState>) -> Result<DbQueryResult, String> {
    let p = Path::new(&path);
    super::file_system::require_in_project(p, &state)?;
    ...
    if is_select {
        ...
        let mut results = stmt.query([]).map_err(|e| format!("Query execution failed: {e}"))?;

        while let Some(row) = results.next().map_err(|e| format!("Error fetching row: {e}"))? {
            let mut row_vals = Vec::with_capacity(num_cols);
            for i in 0..num_cols {
                let val_ref = row.get_ref(i).map_err(|e| format!("Error getting column value: {e}"))?;
                let val = match val_ref {
                    ValueRef::Null => serde_json::Value::Null,
                    ValueRef::Integer(val) => serde_json::Value::Number(serde_json::Number::from(val)),
                    ValueRef::Real(val) => {
