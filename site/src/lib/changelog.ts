...
// Runs at build time (Node, in a Server Component). Reads the real CHANGELOG.md
// so the page always matches the app's release history — nothing to maintain by
// hand here. `scripts/sync-changelog.mjs` (prebuild) refreshes site/CHANGELOG.md
// from the repo root, so the bundled copy is used when the repo root isn't part
// of the deploy context.
...
function readChangelog(): string {
  const candidates = [
    path.resolve(process.cwd(), 'CHANGELOG.md'), // site/CHANGELOG.md (bundled / synced)
    path.resolve(process.cwd(), '../CHANGELOG.md'), // repo root
